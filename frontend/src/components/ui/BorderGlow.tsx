...
function buildMeshGradients(colors: string[]): string[] {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    gs.push(`radial-gradient(at ${GRADIENT_POSITIONS[i]}, ${c} 0px, transparent 50%)`);
  }
  gs.push(`linear-gradient(${colors[0]} 0 100%)`);
  return gs;
}
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  const [edgeProximity, setEdgeProx]   = useState(0);
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    setEdgeProx(getEdgeProximity(card, x, y));
    setCursorAngle(getCursorAngle(card, x, y));
  }, [getEdgeProximity, getCursorAngle]);

  useEffect(() => {
    if (!animated) return;
    const aStart = 110; const aEnd = 465;
    setSweepActive(true);
    setCursorAngle(aStart);
    animateValue({ duration: 500, onUpdate: v => setEdgeProx(v / 100) });
    animateValue({ ease: easeInCubic, duration: 1500, end: 50, onUpdate: v =>
      setCursorAngle((aEnd - aStart) * (v / 100) + aStart) });
    animateValue({ ease: easeOutCubic, delay: 1500, duration: 2250, start: 50, end: 100, onUpdate: v =>
      setCursorAngle((aEnd - aStart) * (v / 100) + aStart) });
    animateValue({ ease: easeInCubic, delay: 2500, duration: 1500, start: 100, end: 0,
      onUpdate: v => setEdgeProx(v / 100),
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: [
          'rgba(0,0,0,.12) 0 1px 2px','rgba(0,0,0,.12) 0 2px 4px',
          'rgba(0,0,0,.12) 0 4px 8px','rgba(0,0,0,.12) 0 8px 16px',
          'rgba(0,0,0,.12) 0 16px 32px','rgba(0,0,0,.12) 0 32px 64px',
        ].join(', '),
      }}
    >
      ...
      {/* content */}
      <div className="flex flex-col relative overflow-auto" style={{ zIndex: 2 }}>
        {children}
      </div>
    </div>
  );
